...
import sdk.chat.demo.robot.adpter.SearchVerseResultAdapter
...
class SearchResultFragment : BaseFragment() {
    ...
    companion object {
        ...
        fun newInstance(source: String): SearchResultFragment {
            ...
            if (!arrayOf("feed", "topic", "question", "favorite", "bible").contains(source)) {
...
//        )
        ...
        swipeRefreshLayout = view.findViewById<LoadMoreSwipeRefreshLayout?>(R.id.swiperefreshlayout)
        if (queryType != "bible") {
...
//        Toast.makeText(applicationContext, "page:${page}", Toast.LENGTH_SHORT).show()
        ...
        if (queryType == "bible") {
            ...
            BibleApiService.searchBibleFromDB(
                searchingKey
            ) { verses ->
                swipeRefreshLayout.isEnabled = false
                swipeRefreshLayout.isRefreshing = false
                if (verses != null) {
                    myAdapter = SearchVerseResultAdapter(onItemClick = { item ->
                        // 处理普通点击
                        activity?.let {
                            BibleActivity.start(
                                activity,
...
//                            recyclerView.scrollToPosition(0);
                        ...
                        adapter.addItems(messages.items as ArrayList<FavoriteList.FavoriteItem>);
                    },
                    { error -> // onError
                        searchingKey = ""
                        (myAdapter as SearchResultAdapter).isLoading = false
                        swipeRefreshLayout?.setLoadingMore(false)
                        swipeRefreshLayout?.isRefreshing = false
                        Toast.makeText(
                            context,
