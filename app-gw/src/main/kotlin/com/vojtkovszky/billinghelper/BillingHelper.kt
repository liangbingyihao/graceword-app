...
//        return p != null
        return purchases.any { it.isAcknowledged && it.purchaseState == Purchase.PurchaseState.PURCHASED }
    }

    fun getSubscriptions(): Purchase? {
    ...
    fun launchPurchaseFlow(
        ...
        if (billingClient.isReady && productDetailsForPurchase != null) {
            ...
            val billingFlowParams = BillingFlowParams.newBuilder().apply {
                ...
                (subscriptionParams ?: SubscriptionPurchaseParams()).getSubscriptionUpdateParams()
                    ?.let {
     ...
     */
    fun initClientConnection(
        queryProductDetailsOnConnected: Boolean = this.queryProductDetailsOnConnected,
        queryOwnedPurchasesOnConnected: Boolean = this.queryOwnedPurchasesOnConnected
    ) {
        if (billingClient.isReady) {
            invokeListener(
                BillingEvent.BILLING_CONNECTED,
                "BillingClient already connected, skipping."
            )
            return
        }
    ...
    private fun invokeListener(
        ...
        Handler(Looper.getMainLooper()).post {
            try {
                if (enableLogging) {
                    Log.d(
                        TAG, "Listener invoked for event $event; message: \"$message\"; " +
        ...
        // query for type on current index
            ...
            billingClient.queryProductDetailsAsync(params.build()) { queryResult: BillingResult, queryProductDetailsListResult: QueryProductDetailsResult ->
                if (queryResult.isResponseOk()) {
                    ...
                    initQueryProductDetailsForTypes(
                        types,
                        currentTypeIndex + 1,
                        productDetailsList,
    ...
    fun getProductNamesForType(productType: String): List<String> {
