...
//        return ChatSDK.db().readReceipt(getId(), user.getId());
    ...
    public MessageSendStatus getMessageStatus() {
        if (this.status != null) {
            return MessageSendStatus.values()[this.status & 0b111];
        }
    ...
    public void setMessageStatus(@NonNull final MessageSendStatus status, final boolean notify) {
        MessageSendStatus oldStatus = getMessageStatus();
        if (oldStatus != status) {
            int newStatus = status.ordinal();
            if (this.status == null) {
                this.status = newStatus;
            } else {
                //两位分别代表状态收藏状态A，状态收藏状态B，后三位代表消息发送状态
                this.status &= 0b11000;
                this.status |= newStatus & 0b111;
            }
    ...
    public boolean isLocalMessage() {
        return entityID == null || entityID.matches("\\d+$");
    }
